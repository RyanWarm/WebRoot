...
class ControllerCommonFileManager extends Controller {
	...
	public function directory() {	
		...
		if (isset($this->request->post['directory'])) {
			$directories = glob(rtrim(DIR_IMAGE . 'uc01_local/') . '/*', GLOB_ONLYDIR); 
			...
			if ($directories) {
				...
				foreach ($directories as $directory) {
					...
					$json[$i]['attributes']['directory'] = utf8_substr($directory, strlen(DIR_IMAGE . 'uc01_local/'));
	...
	public function files() {
		...
		if (!empty($this->request->post['directory'])) {
			$directory = DIR_IMAGE . 'uc01_local' . str_replace('../', '', $this->request->post['directory']);
            		$sub_dir = 'uc01_local' . str_replace('../', '', $this->request->post['directory']) . "/";
		} else {
			$directory = DIR_IMAGE . 'uc01_local';
            		$sub_dir = 'uc01_local/';
		}
		...
		if ($files) {
			foreach ($files as $file) {
				...
				if (in_array(strtolower($ext), $allowed)) {
					...
					$img_url = $this->model_tool_image->resize($sub_dir . basename($file), 50, 50);

					$json[] = array(
						'filename' => basename($file),
						'file'     => utf8_substr($file, strlen(DIR_IMAGE . 'uc01_local/')),
	...
	public function rename() {
		...
		if (isset($this->request->post['path']) && isset($this->request->post['name'])) {
            ...
            $relative_dir = rtrim('data/' . str_replace('../', '', html_entity_decode($this->request->post['path'], ENT_QUOTES, 'UTF-8')), '/');
			$old_name = DIR_IMAGE . $relative_dir;
    ...
    private function conv_filename($orig) {
        $str = urlencode($orig);
        $str = str_replace("%", "", $str);
	...
	public function upload() {
		...
		if (isset($this->request->post['directory'])) {
			if (isset($this->request->files['image']) && $this->request->files['image']['tmp_name']) {
				...
				$filename = $this->conv_filename($filename);

				if ((strlen($filename) < 3) || (strlen($filename) > 255)) {
					$json['error'] = $this->language->get('error_filename');
				}

                		$relative_dir = rtrim('uc01_local' . str_replace('../', '', $this->request->post['directory']), '/');
                
				$directory = DIR_IMAGE . $relative_dir;
		...
		if (!isset($json['error'])) {	
			if (@move_uploaded_file($this->request->files['image']['tmp_name'], $directory . '/' . $filename)) {		
				...
				//if ($this->request->files['image']['width'] > 800 || $this->request->files['image']['height'] > 800) {
                /*
				if ($this->request->files['image']['size'] > 300000) {
                    $json['filename'] = $this->model_tool_image->resize_product_image($relative_dir . '/' . $filename, 800, 800);
                    error_log("resize to :" . $json['filename']);

                } else {
                    $json['filename'] = $relative_dir . '/' . $filename;
                }
                */
                $json['filename'] = $relative_dir . '/' . $filename;
