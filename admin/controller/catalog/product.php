...
class ControllerCatalogProduct extends Controller {
    ...
    private function fillProductForListing( $catalog, $result, $url) {
        $action = array();
  	...
  	private function getList() {				
        ...
        if (isset($this->request->get['lottos'])) {
            $url .= "&lottos=" . $this->request->get['lottos'];
        }
        ...
        $params = array();
        if (isset($this->request->get['lottos']))
            $params['lottos'] = $this->request->get['lottos'];

        if (isset($this->request->get['product_type']))
            $params['product_type'] = $this->request->get['product_type'];
        
		$this->data['insert'] = $this->url->link('catalog/product/insert', 'token=' . $this->session->data['token'] . $url . "&" . http_build_query($params) , 'SSL');
        ...
        if (!isset($this->request->get['lottos']) || $this->request->get['lottos'] == 0) {
            if (!isset($this->request->get['product_type']))
                $data['product_type'] = 'shipping_gift';
            else
            ...
            foreach ($results as $result) {
                $this->fillProductForListing( 'products', $result, $url);
            }
        } else {
            $this->data['lottos'] = array();
            
            $data['product_type'] = 'lotto';
            ...
            foreach ($results as $result) {
                $this->fillProductForListing ( 'lottos', $result, $url);
            }
        }
		...
		$this->data['sort_supplier'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=s.name' . $url, 'SSL');
  	...
  	private function getForm() {
        ...
        if (empty($product_info)) {
            if (isset($this->request->get['lottos']) && $this->request->get['lottos'] == 1) 
                $this->data['product_type'] = 'lotto';
            else {
                if (!isset($this->request->get['product_type']))
                    $this->data['product_type'] = 'shipping_gift';
                else
                    $this->data['product_type'] = $this->request->get['product_type'];
            }
        } else {
		...
		if (isset($this->request->post['orig_image'])) {
			...
			$this->data['orig_image'] = $product_info['orig_image'];
		} else {
		...
		if (!empty($product_info) && $product_info['orig_image'] && file_exists(DIR_IMAGE . $product_info['orig_image'])) {
			$this->data['thumb'] = $this->model_tool_image->resize($product_info['orig_image'], 120, 120);
		} else {
			$this->data['thumb'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		}

		if (isset($this->request->post['feed_image'])) {
			$this->data['feed_image'] = $this->request->post['feed_image'];
		} elseif (!empty($product_info)) {
			$this->data['feed_image'] = $product_info['feed_image'];
		} else {
			$this->data['feed_image'] = '';
		}
		...
		*/
		...
		foreach ($product_images as $product_image) {
			if ($product_image['orig_image'] && file_exists(DIR_IMAGE . $product_image['orig_image'])) {
				$image = $product_image['orig_image'];
			} else {
				$image = 'no_image.jpg';
			}
    	...
    	if (isset($this->request->post['group_count'])) {
			...
			$this->data['group_count'] = $product_info['group_count'];
		} else {
