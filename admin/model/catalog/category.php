...
class ModelCatalogCategory extends Model {
	...
	public function getCategories($parent_id = 0) {
		...
		//if (!$category_data) {
			...
			$query = $this->db->query("SELECT c.*, cd.*, count(p.product_id) as product_count FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) LEFT JOIN product_to_category pc ON (c.category_id = pc.category_id) LEFT JOIN product p ON (pc.product_id = p.product_id AND p.status=1) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' group by c.category_id ORDER BY c.sort_order, cd.name ASC");
    ...
    public function getCategoryProducts($category_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2g ON (p.product_id = p2g.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2g.category_id = '" . $category_id . "' ORDER BY p.status DESC, pd.name ASC");
