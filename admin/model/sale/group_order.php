...
class ModelSaleGroupOrder extends Model {
	...
	public function getOrders($data = array()) {
        $sql = "SELECT o.order_id, o.track_code, o.sender_profile_id, spf.name as sender_name, rpf.name as recv_name, o.recv_profile_id, o.activity_status, o.ship_status, o.address_id, o.participants_count, o.group_count, o.date_added, o.activity_end_date, o.product_id, pd.name as product_name FROM `" . DB_PREFIX . "group_order` o LEFT JOIN profile AS spf ON (o.sender_profile_id=spf.id AND o.sender_profile_network=spf.network) LEFT JOIN profile AS rpf ON (o.recv_profile_id = rpf.id AND o.recv_profile_network AND rpf.network) LEFT JOIN product as p ON (o.product_id = p.product_id) LEFT JOIN product_description AS pd ON (p.product_id = pd.product_id AND pd.language_id=2) ";

        $sql .= "WHERE TRUE ";

		if (isset($data['filter_activity_status']) && !is_null($data['filter_activity_status'])) {
			$sql .= " AND o.activity_status = '" . $data['filter_activity_status'] . "'";
		}

		if (isset($data['filter_ship_status']) && !is_null($data['filter_ship_status'])) {
			$sql .= " AND o.ship_status = '" . $data['filter_ship_status'] . "'";
		}
		...
		if (!empty($data['filter_track_code'])) {
			$sql .= " AND o.track_code = '" . $data['filter_track_code'] . "'";
		}

		if (!empty($data['filter_sender_name'])) {
			$sql .= " AND spf.name LIKE '%" . $this->db->escape($data['filter_sender_name']) . "%'";
		}

		if (!empty($data['filter_recv_name'])) {
			$sql .= " AND rpf.name LIKE '%" . $this->db->escape(($data['filter_recv_name'])) . "%'";
		}
		...
		if (!empty($data['filter_activity_end_date'])) {
			$sql .= " AND DATE(o.activity_end_date) = DATE('" . $this->db->escape($data['filter_activity_end_date']) . "')";
		}
	...
	public function getTotalOrders($data = array()) {
      	$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "group_order` as o LEFT JOIN profile AS spf ON (o.sender_profile_id=spf.id AND o.sender_profile_network=spf.network) LEFT JOIN profile AS rpf ON (o.recv_profile_id = rpf.id AND o.recv_profile_network AND rpf.network) ";
