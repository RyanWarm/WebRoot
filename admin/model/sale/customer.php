...
class ModelSaleCustomer extends Model {
	...
	public function getCustomers($data = array()) {
        $sql = "SELECT u.id as id, p.id as profile_id, p.network as network, p.name as name, p.gender as gender, p.birthday as birthday, p.country as country, p.province as province, p.city as city, p.profile_image as image, p.work_history as work, p.schools as schools, u.created_at as created_at FROM user as u, profile as p ";

		$implode = array();
        $implode[] = " u.id=p.user_id ";

		if (!empty($data['filter_id'])) {
			$implode[] = "u.id=" . $data['filter_id'];
		}	
		...
		if (!empty($data['filter_email'])) {
			$implode[] = "p.id=" . $data['filter_email'];
		}

		if (!empty($data['filter_customer_group_id'])) {
			$implode[] = "p.network = '" . $data['filter_customer_group_id'] . "'";
		}	
			
		if (isset($data['filter_status']) && !is_null($data['filter_status'])) {
			$implode[] = "p.gender = '" . (int)$data['filter_status'] . "'";
		}
        ...
        */
        ...
        for ($i=0;$i<count($res);$i++) {
            if ($res[$i]['network'] == '1') {
                $res[$i]['network_name'] = 'Weibo';
            }   
            else if ($res[$i]['network'] == '2') {
                $res[$i]['network_name'] = 'Renren';
            } else {
                $res[$i]['network_name'] = 'Unknown';
            }   
        }   
		...
		if ($address_query->num_rows) {
            ...
            */
			...
			return array(
				...
				'user_name'      => $address_query->row['user_name'],
				...
				'zone'           => $zone,
				'zone_code'      => $zone_code,
                'province'       => $address_query->row['province'],
	...
	public function getAddresses($customer_id) {
		...
		$query = $this->db->query("SELECT address_id FROM " . DB_PREFIX . "address WHERE user_id = '" . (int)$customer_id . "'");
    ...
    public function getNetworks() {
        $sql = "SELECT distinct(network) from profile order by network";
        $query = $this->db->query($sql);
        $res = $query->rows;
        for ($i=0;$i<count($res);$i++) {
            if ($res[$i]['network'] == '1') {
	...
	public function getTotalCustomers($data = array()) {
      	$sql = "SELECT COUNT(*) AS total FROM user as u, profile as p ";
		
		$implode = array();

        $implode[] = "p.user_id=u.id";
